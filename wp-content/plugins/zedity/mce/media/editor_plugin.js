...
/*
 * Modified to allow Zedity iframe preview.
 */
...
(function() {
	...
	tinymce.create('tinymce.plugins.NewMediaPlugin', {
		init : function(ed, url) {
			...
			ed.onPreInit.add(function() {
				...
				ed.parser.addNodeFilter('object,embed,video,audio,script,iframe', function(nodes) {
					...
					while (i--) {
						...
						if (parent && parent.attributes.map.class && parent.attributes.map.class.indexOf('zedity-iframe-wrapper')==-1) {
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(plugin, menu, element) {
						if (element.nodeName === 'IMG' && element.className.indexOf('mceItemMedia') !== -1)
							menu.add({title : 'media.edit', icon : 'newmedia', cmd : 'mceMedia'});
					});
				}
			});
			...
			// Register buttons
			ed.addButton('newmedia', {title : 'media.desc', cmd : 'mceMedia'});

			// Update media selection status
			ed.onNodeChange.add(function(ed, cm, node) {
				cm.setActive('newmedia', isMediaImg(node));
			});
		},
	...
	// Register plugin
	tinymce.PluginManager.add('newmedia', tinymce.plugins.NewMediaPlugin);
})();
