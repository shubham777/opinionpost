...
/*
 * Modified to disallow Zedity content editing.
 */
...
(function() {
				...
				function isNextPrevTreeNodeNonEditable() {
					...
					while (node = (backspace ? walker.prev() : walker.next())) {
						...
						if (getContentEditable(node) === "false") {
							//ensure is not a Zedity content
							if (!node.hasAttribute('class') || node.getAttribute('class').indexOf('zedity')==-1) {
	...
	tinymce.create('tinymce.plugins.NewNonEditablePlugin', {
			...
			ed.onPreInit.add(function() {
				...
				ed.parser.addAttributeFilter('class', function(nodes) {
					...
					while (i--) {
						...
						} else if (className.indexOf('zedity-wrapper')!==-1) {
							node.attr(internalName, "false");
						}
					}
				});
	...
	// Register plugin
	tinymce.PluginManager.add('newnoneditable', tinymce.plugins.NewNonEditablePlugin);
})();